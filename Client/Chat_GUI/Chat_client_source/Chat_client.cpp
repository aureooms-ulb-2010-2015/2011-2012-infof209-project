...
Chat_client::Chat_client():__pseudo("default"),__channel("default"),__server("localhost"),__port(APP_PORT){
    this->__socket = new ClientSocket(this->server(),this->port());
    this->__receiver = new Chat_client_message_receiver(this);
    this->__sender = new Chat_client_message_sender(this);
}
